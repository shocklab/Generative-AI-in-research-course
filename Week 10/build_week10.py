...
SL1_BODY = """
  <div class="intro-text">
    ...
    <p>This sub-lesson gives you that definition (an agent is a model wrapped in a <em>harness</em> of tools, memory, loops, and permissions) and then frames the whole week around one shift: <strong>since 2024, the harness has become the product</strong>. The base model matters less than it used to; how it is scaffolded matters more. We will look at the May 2026 agent landscape, the trend of coding agents being repurposed for general knowledge work, and what all of this means for a working researcher.</p>
  ...
  <ul class="styled-list">
    <li><strong>A model</strong>: the same kind of transformer doing next-token prediction we met in Week 2. Nothing about the underlying architecture has changed; the model is still predicting tokens. What is new is what those tokens are allowed to <em>do</em>.</li>
  </ul>

  <div class="technical-detail">
    <h4>&#129520; The Harness &mdash; everything built around the model</h4>
    <p style="color: #555; line-height: 1.7; margin-bottom: 15px;">The model is the engine; the harness is the chassis, wheels, steering, and brakes built around it. A model on its own answers questions. A model in a harness gets things done. These four components, plus the prompts and context-management that coordinate them, are the harness:</p>
    <ul class="styled-list" style="margin-top: 0;">
      <li><strong>Tools</strong>: functions the model can call by emitting a structured request in its output stream: run code, search the web, read a file, query a database, send an email. The model does not execute these; the harness does, and feeds the result back in.</li>
      <li><strong>A loop</strong>: the harness runs the model, executes any tool call it requests, appends the result to the context, and runs the model again. This repeats until the model decides the task is done. A chatbot answers once; an agent iterates.</li>
      <li><strong>Memory</strong>: some way of carrying state across many loop iterations (and sometimes across sessions), because a useful task can span dozens or hundreds of steps that overflow a single context window.</li>
      <li><strong>Permissions</strong>: the boundary of what the agent is allowed to do without asking. Read-only? Allowed to write files? Allowed to spend money? This is the safety dial that matters most, and the one researchers most often leave on the wrong setting.</li>
    </ul>
  </div>

  <p style="color: #555; line-height: 1.75; margin-top: 20px; margin-bottom: 15px;">Keep that word, <em>harness</em>, in mind. In 2026 the harness, not the engine, increasingly decides how well the whole thing drives, which is the subject of the next section.</p>
  ...
  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">This definition lets us draw two distinctions. A <strong>chatbot with browsing</strong> is not yet an agent in the full sense: it makes one tool call and answers. A <strong>2023-style &ldquo;chain&rdquo;</strong> (a fixed, pre-scripted sequence of model calls) is not an agent either, because the model does not decide what to do next; the developer hard-coded the steps. An agent chooses its own next action inside the loop. The model, not the developer, steering the loop is what changed between 2023 and 2026, and it is where both the new capability and the new failure modes come from.</p>

  <h2 class="section-title">&#128295; What&#39;s New Since 2024: The Harness Is the Product</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">In 2023, agent performance was dominated by the base model: a better model gave you a better agent, full stop. By May 2026, that is no longer true. Agent performance is a <em>joint</em> property of the model, the harness around it, and the strategy for packing the right context into the model at each step. The harness (the prompts, the tool definitions, the context management, the evaluation loop, the model-specific tuning) has become the thing that companies actually build and sell.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">This is not only a claim from practitioners. It is now the subject of formal research. A March 2026 Stanford preprint, <em>Meta-Harness: End-to-End Optimization of Model Harnesses</em> (Lee et al.), opens with it:</p>
  ...
  <div class="technical-detail">
    ...
    <p style="color: #555; line-height: 1.7;">The paper builds a system that automatically searches over harness code, keeping the model fixed, and improves on a state-of-the-art context-management baseline by <strong>7.7 points while using four times fewer context tokens</strong>. A measurable chunk of an agent&#39;s performance lives in the harness, independent of the model, and is large enough to optimise on its own.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The companies building these systems show the same effect in detail. In a February 2026 engineering write-up, the LangChain team took a single coding model (GPT-5.2-Codex) and, <em>without changing the model</em>, improved their agent&#39;s score on <em>Terminal-Bench 2.0</em> by 13.7 points, from 52.8% to 66.5%, moving it from outside the top 30 to the top 5. Every point of that gain came from the harness:</p>
  ...
  <div class="case-study">
    ...
    <ul class="styled-list">
      <li><strong>A better system prompt</strong>: explicit planning, build, verify, and fix phases, with emphasis on edge-case testing and checking output against the task specification.</li>
      <li><strong>Middleware in the loop</strong>: a pre-exit checklist that forces verification before the agent declares itself done, an onboarding step that maps the working environment, and a loop-detector that interrupts the agent when it gets stuck repeating an edit.</li>
      <li><strong>Context management</strong>: mapping the directory structure and available tools up front, and warning the agent about its remaining time budget.</li>
      <li><strong>A &ldquo;reasoning sandwich&rdquo;</strong>: allocating more reasoning effort to the planning and verification phases than to the execution in between.</li>
    </ul>
    <p style="margin-top: 12px;">None of this touched the model&#39;s weights. A large slice of agent performance is engineering of the scaffolding, and it is the kind of thing a tool vendor can do that you, choosing between tools, cannot see from the model name alone.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">And it generalises across models. In a May 2026 follow-up the same team reported that, on a different agentic benchmark (tau2-bench), prompts and middleware alone moved scores by <strong>10 to 20 points</strong> (a roughly 20% lift on one coding model and 10% on another), and that the same harness techniques apply when driving open-weight models such as Kimi, Qwen, and DeepSeek (which we return to in 10.5). The lesson holds regardless of whose model you use: a large, measurable share of the work happens in the harness.</p>

  <div class="warning-box">
    <h4>&#9888;&#65039; What this means when you read a benchmark</h4>
    <p>If the harness can swing a score by ten points or more, then a leaderboard number attached to a bare model name is close to meaningless without knowing the scaffolding that produced it. This is not hypothetical: <em>Terminal-Bench 2.0</em> (the benchmark itself, arXiv:2601.11868) is run through a named harness, and different harnesses on the same model produce different scores. When a tool claims &ldquo;state-of-the-art on benchmark X&rdquo;, ask: <em>which harness?</em> A strong model, badly scaffolded, will underperform a weaker model in a better harness. This is the agentic version of the Week 9 lesson that benchmark numbers need their context to mean anything.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">There is a research-tooling consequence too. The agent-infrastructure companies are now shipping harnesses as products: at LangChain&#39;s <em>Interrupt</em> conference (13&ndash;14 May 2026), the company released a cluster of harness-building tools: a hosted runtime for &ldquo;deep agents&rdquo;, a faster trace database, sandboxes, a context-management hub, and a gateway that enforces spend limits and redacts personal data before requests leave your environment. You probably need none of these. What they show is that the centre of gravity in the field has moved from &ldquo;which model&rdquo; to &ldquo;which harness&rdquo;. When you choose an agent tool, you are choosing a harness as much as a model.</p>
    ...
    <table class="comparison-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>Drive a graphical interface (click, type, scroll) the way a person would, for tasks with no API.</td>
  ...
  <p style="color: #555; line-height: 1.75; margin-top: 20px; margin-bottom: 15px;">A concrete, datable example of how fast the creative corner is moving: on 28 April 2026 Anthropic shipped nine creative tool connectors for Claude, including Blender, Ableton, Adobe and Autodesk integrations, and made them available on the <em>free</em> plan. We keep asking &ldquo;what is actually free&rdquo; throughout this week, because for many students at UCT it is the question that decides whether a tool is usable at all.</p>
  ...
  <div class="case-study">
    ...
    <p>Two honest caveats. First, these are <em>first-party</em> claims, the person shipping the product describing his own product, not independent measurements, so treat the 42% as a vendor figure. Second, &ldquo;as fast as a person&rdquo; is about <em>speed</em>, not <em>reliability</em>; a fast agent that takes a wrong action quickly is not obviously progress (we develop this distinction in 10.2). But the qualitative marker is real and worth recording: as of mid-2026, an AI driving a graphical interface by clicking and typing is no longer visibly slow. The bottleneck has moved from speed to judgement.</p>
  </div>

  <div class="info-box">
    <h4>&#128221; A note on naming</h4>
    <p>Throughout this course we refer to model <em>families</em> (&ldquo;Claude (family)&rdquo;, &ldquo;GPT (family)&rdquo;, &ldquo;Gemini (family)&rdquo;) rather than chasing version numbers, because the versions change monthly. Agent <em>products</em> are different: &ldquo;Claude Code&rdquo;, &ldquo;Codex&rdquo;, &ldquo;Cursor&rdquo;, &ldquo;Manus&rdquo; are tool names, and we keep them, the same way we kept &ldquo;Whisper large-v3&rdquo; in Week 8. When you see a specific version number attached to a benchmark figure, treat it as a dated citation, not a recommendation.</p>
  </div>

  <h2 class="section-title">&#128640; Agents Are Breaking Containment</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">A throughline of April&ndash;May 2026 is that the families above are bleeding into one another. Tools built as <em>coding</em> agents are being pointed at general knowledge work, because what makes a good coding agent (a solid loop, file access, the ability to run for a long time without losing the thread) is also what a good general agent needs.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">In the space of a few weeks, the major coding-agent products launched explicit &ldquo;for work&rdquo; or &ldquo;for non-code tasks&rdquo; surfaces. OpenAI&#39;s 16 April 2026 Codex update, which the company itself titled <a href="https://openai.com/index/codex-for-almost-everything/" target="_blank" rel="noopener">&ldquo;Codex for (almost) everything&rdquo;</a>, repositioned a coding tool for knowledge work: operating your computer, working across apps like Slack, Gmail and Notion, remembering preferences, and scheduling its own future work. Anthropic pushed Claude into knowledge-work and creative settings over the same period. (A reported general-work product circulating under the name &ldquo;Orbit&rdquo; had not, as of mid-May 2026, been officially launched; treat it as a rumour until it ships.) The pattern, though, is on the record: the coding harness has become a general-purpose work harness.</p>
  ...
  <div class="highlight-box">
    ...
    <p>Hold onto that. In 10.2 we apply the Week 9 patched / reduced / structural failure taxonomy to agents specifically, because agents fail in genuinely new ways that single-turn chatbots did not.</p>
  </div>
  ...
  <div class="resource-placeholder">
    ...
    <p style="margin-top: 12px;"><strong>For day-by-day tracking</strong> of these fast-moving developments, the <a href="https://www.latent.space/" target="_blank" rel="noopener">AINews / Latent Space</a> daily briefings are a useful aggregator, but note that they themselves aggregate company announcements and social-media posts, so trace any specific number back to its origin (as we have done here) before citing it. For a pragmatic practitioner&#39;s voice, including healthy scepticism about how much of this you actually need, <a href="https://simonwillison.net/tags/model-context-protocol/" target="_blank" rel="noopener">Simon Willison&#39;s ongoing writing</a> is consistently worth reading.</p>
  </div>

  <div class="intro-text" style="margin-top: 50px;">
    <h2>&#128073; What Comes Next</h2>
    <p><strong>Sub-Lesson 10.2 &mdash; Failure Modes for Multi-Step / Long-Horizon Tasks.</strong> Now that we have a working definition and the &ldquo;harness is the product&rdquo; frame, we turn to where agents break. We take the patched / reduced-but-persistent / structural taxonomy from Week 9.2 and apply it directly to agents, because running a model in a loop for many steps introduces failure modes that no single-turn chatbot ever had. The headline finding, from a February 2026 Princeton study: across 18 months and 14 models, agent <em>accuracy</em> improved substantially while agent <em>reliability</em> barely moved. Accuracy is not reliability, and for a researcher the gap between them is where the danger lives.</p>
  </div>
"""
...
SL2_BODY = """
  <div class="intro-text">
    ...
    <p>One distinction runs through the whole sub-lesson: <strong>reliability is not accuracy</strong>. An agent can be more accurate than ever, better at any individual step, and still be unreliable, because reliability is about doing the whole multi-step task consistently, recovering from its own mistakes, and knowing when it is wrong. A February 2026 Princeton study put numbers on this: across 14 models and 18 months, accuracy improved substantially while reliability barely moved. For a researcher deciding whether to hand a task to an agent, the gap between those two things is where the danger lives.</p>
    <p>As always, the framing is the durable part and the examples are the snapshot. The specific failure stories below will date; the three-category question, <em>is this patched, reduced, or structural?</em>, will not.</p>
  </div>

  <h2 class="section-title">&#9201;&#65039; Why Agents Need Their Own Failure Lesson</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">A chatbot makes one prediction and stops. If it is right 90% of the time, that is roughly your experience of it. An agent runs the model in a loop, dozens or hundreds of times for a single task, and three things change.</p>

  <ul class="styled-list">
    <li><strong>Errors compound.</strong> If each step is 95% reliable and the task takes twenty independent steps, the probability that <em>every</em> step is correct is roughly 0.95<sup>20</sup>, about 36%. High per-step accuracy can still produce a task that usually fails. This is the agentic version of the Week 7 silent-error problem: each step looks plausible, the end result is wrong.</li>
    <li><strong>State accumulates.</strong> The agent carries its own earlier outputs forward as context. An early mistake does not just cost one step; it poisons every step that depends on it, and the agent rarely notices.</li>
    <li><strong>The agent acts.</strong> A chatbot that hallucinates wastes your time. An agent that hallucinates can delete a file, send an email, or spend money, because it has tools and permissions (the harness components from 10.1). The cost of a failure is no longer just a wrong answer.</li>
  </ul>
  ...
  <ul class="styled-list">
    ...
    <li><strong>&ldquo;Forgetting&rdquo; available tools.</strong> Better system prompts and tool-discovery steps (the harness changes in the LangChain worked example from 10.1) have substantially reduced this.</li>
  ...
  <div class="card-grid">
    <div class="card">
      ...
      <p>Agents get stuck repeating an edit, re-running a failing command, or oscillating between two states. Harnesses now ship explicit loop-detectors (again, see the LangChain recipe in 10.1), which tells you the problem is real enough to need a dedicated mitigation.</p>
    </div>
    <div class="card">
  ...
  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">These follow from how agents work, not from fixable engineering gaps. They are the ones to design your workflow <em>around</em>, because the next model release will not remove them. Each gets its own section below: the reliability gap, long-horizon planning collapse, and prompt injection. Compositional brittleness sits here too: the compounding-error problem from the top of this lesson, and the same effect behind ProgramBench scoring 0% on whole-repository tasks in Week 9.</p>

  <h2 class="section-title">&#128201; Reliability Is Not Accuracy: The Princeton Finding</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">The structural case is made in detail by a February 2026 paper from Princeton&#39;s Center for Information Technology Policy: <em>Towards a Science of AI Agent Reliability</em> (Rabanser, Kapoor, Kirgis, Liu, Utpala &amp; Narayanan). It is the agent-era counterpart to the Kalai et al. hallucination paper we met in Week 9, arguing that a whole class of failure is structural, not a passing engineering problem.</p>
  ...
  <div class="highlight-box">
    ...
    <p>They decompose reliability into four dimensions (<strong>consistency, robustness, predictability, and safety</strong>) measured by twelve concrete metrics. The weakest, across the board, is <strong>predictability</strong>: <em>&ldquo;agents are not good at knowing when they&#39;re wrong.&rdquo;</em> On one benchmark, most models could not distinguish their own correct answers from their incorrect ones better than chance.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">Two numbers make this concrete. Outcome <strong>consistency</strong> scores ranged from <strong>30% to 75%</strong> across the models tested, meaning that running the same agent on the same task under identical conditions often produced different outcomes. And because predictability is so weak, you cannot use the agent&#39;s own confidence as a filter: the &ldquo;ask it only when it&#39;s sure&rdquo; strategy fails, because it is not reliably sure when it should be.</p>
  ...
  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Why do long tasks fail so distinctively? A January 2026 paper, <em>Why Reasoning Fails to Plan</em> (Wang et al.), gives a structural answer that explains a failure you will see repeatedly.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The argument: the step-by-step reasoning that makes models good at short tasks is, formally, a <em>greedy</em> strategy that picks the locally best next step. Greedy strategies are fine over short horizons. Over long ones they fail, because an early choice that looks locally optimal commits the agent to a path whose costs only show up much later, and those early commitments get <em>amplified over time and are difficult to recover from</em>. The authors prove the gap is <strong>structural, not a matter of model capability</strong>: a more powerful reasoner making step-wise decisions is still fundamentally limited on long-horizon tasks.</p>

  <div class="case-study">
    <h4>&#128161; The counter-intuitive result: small + planning beats large + greedy</h4>
    <p>One demonstration stands out: with their lookahead method (FLARE), <em>&ldquo;LLaMA-8B with FLARE frequently outperforms GPT-4o with standard step-by-step reasoning&rdquo;</em> on long-horizon benchmarks. A small model that genuinely plans can beat a much larger model that reasons step by step.</p>
    <p>The research lesson is about <em>reasoning not being planning</em>, not about using LLaMA-8B. When you see an agent handle each individual step competently and still walk the task into a wall, you are watching greedy step-wise reasoning fail at planning: a structural limitation, not a bug that more model scale will fix.</p>
  </div>
  ...
  <div class="card-grid">
    <div class="card">
      ...
      <p style="font-size: 0.95em; color: #003A70;">Top result: Claude Opus 4.6 at $1.27M average final funds, with the much cheaper GLM-5 (from China&#39;s Zhipu AI) close behind at $1.21M for roughly 11&times; lower inference cost. A preview of the lower-cost and Chinese-model story we develop in 10.5.</p>
    </div>
    <div class="card">
      <h3>The &ldquo;Meltdown Onset Point&rdquo;</h3>
      <p><em>Beyond pass@1: A Reliability Science Framework for Long-Horizon LLM Agents</em> (arXiv:2603.29231) evaluated 10 models across 23,392 episodes and introduced metrics for how agents fall apart over time, including a <strong>Meltdown Onset Point</strong> that detects behavioural collapse by watching the agent&#39;s tool-call patterns become erratic.</p>
  ...
  <div class="info-box">
    ...
    <p>Prompt injection and the permissions dial are not abstractions when an agent touches real systems. An agent with access to a university inbox, a grant portal, or institutional data is acting inside systems governed by South Africa&#39;s Protection of Personal Information Act (POPIA). The structural insecurity of agents is one more reason the verification and permissioning burden cannot be delegated, a theme we return to in the Week 10 activity, where you will be asked to state where your tools send data before you use them.</p>
  </div>

  <h2 class="section-title">&#129519; Putting the Taxonomy to Work</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">As in Week 9, the point of the taxonomy is action. When you observe an agent failure, locate it; the category tells you what to do.</p>
...
SL3_BODY = """
  <div class="intro-text">
    <h2>&#127919; What We&#39;ll Cover</h2>
    <p>This is the practical sub-lesson: a tour of the agent tools that actually exist in May 2026, organised by what they do. We cover three families here: coding agents, computer-use and browser agents, and the connector layer (MCP) that links agents to your data. The fourth family, research agents (&ldquo;Deep Research&rdquo; modes), gets its own treatment in 10.4, and the full honest free-versus-paid tool comparison, including Chinese options, lands in 10.5.</p>
  ...
  <div style="overflow-x: auto;">
    <table class="comparison-table">
      ...
      <tbody>
        ...
        <tr>
          <td>Late 2025</td>
          <td>Simular Agent S2 (first agent to cross the human baseline)</td>
          <td>72.6%</td>
        </tr>
        <tr>
          <td>March 2026</td>
  ...
  <div class="warning-box">
    ...
    <p>It is true and striking that computer-use agents now match or exceed the average human score on OSWorld. But read that claim through 10.2. OSWorld is 369 specific, well-defined tasks, a narrow slice of what &ldquo;using a computer&rdquo; means. A benchmark score is the kind of single-number summary that the Princeton reliability work warns hides operational flaws. In real, open-ended workflows, computer-use agents remain fragile: they misclick, they lose the thread on long tasks, and (per 10.2) their outcome consistency is far below 100%. The summary is the one from 10.1: the bottleneck has moved from speed to judgement, not disappeared.</p>
  </div>
  ...
  <div class="warning-box">
    ...
    <p>This is 10.2&#39;s structural security hole at its most exposed. An agentic browser reads whatever web page you send it to, and any page can contain hidden instructions aimed at your agent. Never give a browsing agent the ability to act on accounts that matter (email, banking, institutional systems) while it roams the open web. Keep the permissions dial low: let it read and summarise, not send, buy, or delete.</p>
  </div>

  <h2 class="section-title">&#129300; Manus and the &ldquo;General Agent&rdquo; Claim</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Manus is a useful case study in reading agent hype critically. Launched in invitation-only beta in March 2025 by Butterfly Effect (a Chinese-founded company that relocated to Singapore), it was marketed as a <em>general</em> autonomous agent: give it a goal, walk away, come back to a finished task. The launch demos generated enormous attention and a thriving secondary market in invitation codes.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">Apply the week&#39;s discipline. Manus&#39;s headline capability claims are <strong>company-reported</strong>, and independent replication has been scarce, the situation 10.1 and 10.2 tell you to treat cautiously, because a polished demo is a best case, not a reliability measurement. What is independently documented is the regulatory drama: in April 2026, China&#39;s National Development and Reform Commission <strong>blocked Meta&#39;s reported $2&nbsp;billion acquisition of Manus</strong>, following a Ministry of Commerce investigation earlier in the year. That is a verifiable fact about the company; the &ldquo;it can do anything&rdquo; framing is not. It is a real tool with genuine engineering behind it, and a reminder to separate what a vendor claims from what has been measured.</p>

  <h2 class="section-title">&#128268; MCP: The Connector Layer</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">The Model Context Protocol (MCP) is the plumbing that lets an agent connect to your tools and data through a single standard interface, often described as &ldquo;USB-C for AI&rdquo;. Instead of every tool needing a bespoke integration with every model, a tool exposes an MCP server once and any MCP-speaking agent can use it.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">MCP matters in 2026 because it stopped being one vendor&#39;s idea and became a cross-industry standard with remarkable speed. Anthropic introduced it in November 2024; OpenAI adopted it in March 2025; Google followed in April 2025. In December 2025 Anthropic donated the protocol to a Linux Foundation body (the Agentic AI Foundation, co-founded with Block and OpenAI), so it is now governed in the open rather than controlled by one company. In barely a year it went from a single company&#39;s proposal to a standard that all the major model providers speak, which is why it is now a researcher&#39;s concern and not only a developer&#39;s.</p>

  <div class="info-box">
    <h4>&#128300; Why a researcher should care</h4>
    <p>MCP is what will connect agents to the tools you already use. MCP servers exist (or are emerging) for reference managers like <strong>Zotero</strong>, for <strong>GitHub</strong>, <strong>Google Drive</strong>, <strong>Notion</strong>, and more, so an agent can search your library, read your repository, or draft in your notes through one standard. On Claude.ai, a set of connectors is available even on the <em>free</em> plan (the nine creative connectors from 10.1 are MCP under the hood). We return to the specific research-relevant connectors, and how to use them safely, in 10.5.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The protocol&#39;s own March 2026 roadmap signals where it is heading: scaling the transport layer for production, refining agent-to-agent communication, maturing open governance, and enterprise features like audit trails and authentication. That evolution &ldquo;from releases to working groups&rdquo; marks MCP&#39;s shift from experiment to infrastructure.</p>

  <div class="warning-box">
    <h4>&#9888;&#65039; The pragmatic and the security caveats</h4>
    <p>Two cautions. First, practically: MCP is not always the right tool. Simon Willison, among the most careful practitioner voices, has argued that for a lot of agent work, plain command-line utilities are simpler and more reliable than wiring up an MCP server. MCP is a standard, not a magic wand. Second, and more seriously: anything an agent reads through MCP is untrusted content, so MCP is another channel for the prompt injection from 10.2. A connector that can both read your email and browse the web is the capability combination that turns a hidden instruction on a web page into an action on your inbox. Connect deliberately; keep the permissions dial low.</p>
  </div>

  <h2 class="section-title">&#129517; Reading the Frontier: Real, Overclaimed, and Vapour</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">A closing honesty note. In the spirit of Week 9, the honest move is calibration in <em>both</em> directions, not blanket scepticism. Some frontier agent claims are genuine and shipping; some are real but overclaimed; some are still vapour. The skill is telling them apart. Three examples that are easy to get wrong in either direction.</p>
  ...
  <div class="case-study">
    ...
    <p>What is overclaimed is the framing &ldquo;the AI did the science&rdquo;. The workshop paper passed a lower bar than a main conference and was withdrawn by agreement; the system still makes errors a reviewer must catch. Most of these tools <em>generate hypotheses or optimise within a well-defined space</em>; the human still frames the problem, validates the result, and supplies the novel data the agent cannot gather itself. As one assessment put it, we are likely years from autonomous &ldquo;true innovation&rdquo;. So: real and accelerating, genuinely useful, and not the end-to-end autonomous discovery the headlines imply. Both halves are true.</p>
  </div>

  <div class="case-study">
    <h4>&#128260; Self-improving agents: bootstrapping is real; recursive self-improvement is not (yet)</h4>
    <p>Here too the honest answer is &ldquo;partly real&rdquo;. AI coding tools are now substantially used to build themselves: Anthropic engineers write much of Claude Code <em>using</em> Claude Code, and the Meta-Harness research from 10.1 automates the search for better harnesses. That is genuine bootstrapping: the tools are accelerating their own development, and it is not hype to say so.</p>
  ...
  <p style="color: #555; line-height: 1.75; margin-top: 20px; margin-bottom: 15px;">And some things genuinely are just vapour or rumour as of mid-May 2026, for example the Anthropic general-work product circulating under the codename &ldquo;Orbit&rdquo; (from 10.1), which had not officially launched. A codename is not a product, and a demo is not a deployment. The pattern across all three examples is the same: find the verified core, name the overclaim around it, and do not let either the hype or the backlash do your thinking for you.</p>

  <div class="highlight-box">
    <h3>How to read this whole landscape</h3>
    <p>Three questions cut through almost any agent-tool announcement. <strong>Which harness?</strong> (10.1: the model name alone tells you little.) <strong>How reliable, not just how accurate?</strong> (10.2: a demo is a best case.) <strong>What can I actually access for free, from here?</strong> (this week&#39;s throughline.) Hold those three up against every tool below and in 10.5, and you will not be badly misled.</p>
  </div>
  ...
  <div class="intro-text" style="margin-top: 50px;">
    <h2>&#128073; What Comes Next</h2>
    <p><strong>Sub-Lesson 10.4 &mdash; RAG in 2026.</strong> We deferred the research-agent family to its own lesson, and here it is. Retrieval-augmented generation was the backbone of the literature tools in Week 5; we look at what has changed since: how much longer context windows have eroded the need for naive retrieval, where &ldquo;agentic RAG&rdquo; wins it back, and why evaluating any of this remains genuinely hard. Deep Research modes, the research agents, sit at the centre of that story.</p>
  </div>
"""
...
SL4_BODY = """
  <div class="intro-text">
    <h2>&#127919; What We&#39;ll Cover</h2>
    <p>Week 5 introduced retrieval-augmented generation (RAG) as the engine behind the AI literature tools: the technique of fetching relevant documents and feeding them to a model so its answer is grounded in real sources rather than its training data alone. Those tools and that technique are still central. But three things have shifted since Week 5, and this sub-lesson is about all three.</p>
    <p>First, <strong>long context</strong>: frontier models now routinely accept a million tokens or more, which changes when you need retrieval at all. Second, <strong>agentic RAG</strong>: retrieval has moved from a single fetch-then-answer step to a loop in which the model plans, retrieves, reads, and retrieves again, which is what the Deep Research modes you have heard about are doing. Third, <strong>evaluation</strong>: we now have frameworks for measuring whether a RAG system is any good, though the hardest cases remain open.</p>
    <p>The verdict the lesson builds to: for reading one long document, just use long context; for synthesising across many sources, agentic RAG still wins; and for anything you will cite, the Week 5 verification protocols apply regardless of which method produced the answer. None of this removes the researcher from the loop.</p>
  </div>

  <h2 class="section-title">&#128214; Recap: What Week 5 Established</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Week 5 mapped a landscape of AI literature tools (Elicit, Consensus, NotebookLM, Perplexity, Connected Papers, ResearchRabbit) and a hard warning: the hallucinated-citation crisis is real. Both still hold in 2026. The tools have improved; the warning has not expired.</p>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">In the Week 9 taxonomy, hallucinated citations are a <em>reduced-but-persistent</em> failure: far less common on well-covered topics than in 2023, but still present, and concentrated where research lives, in the niche, the recent, the long-tail. RAG was always partly a response to this: ground the model in retrieved sources and it has less need to invent them. That logic is intact. What has changed is how the grounding is done.</p>
  ...
  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The most-cited study on this is Li et al. (2024), <em>Retrieval Augmented Generation or Long-Context LLMs?</em> (Google; EMNLP 2024). Their finding, in their words: <em>&ldquo;when resourced sufficiently, [long context] consistently outperforms RAG in terms of average performance. However, RAG&#39;s significantly lower cost remains a distinct advantage.&rdquo;</em> They proposed a hybrid, &ldquo;Self-Route&rdquo;, that routes each query to whichever approach suits it, cutting cost while keeping long-context quality.</p>

  <div class="warning-box">
    <h4>&#9888;&#65039; Two caveats on the long-context win</h4>
    <p>First, that study is from 2024, tested on the long-context models of the time. Treat it as evidence of the <em>shape</em> of the trade-off, not the current margins; the Week 9 &ldquo;which model, which date&rdquo; discipline applies.</p>
  ...
  <ul class="step-list">
    <li><strong>Plan</strong>: break the question into sub-questions.</li>
    <li><strong>Retrieve</strong>: search for sources on the first sub-question.</li>
    <li><strong>Read and reflect</strong>: work out what was learned and what is still missing.</li>
    <li><strong>Retrieve again</strong>: run follow-up searches based on what the first round turned up (this is the step classic RAG cannot do).</li>
    <li><strong>Synthesise and check</strong>: assemble an answer and, in better systems, verify claims against the retrieved sources.</li>
  </ul>

  <p style="color: #555; line-height: 1.75; margin-top: 15px; margin-bottom: 15px;">This loop is the agent definition from 10.1 (model + tools + loop + memory) pointed at a retrieval tool. And it is not a research curiosity: <strong>the &ldquo;Deep Research&rdquo; modes</strong> in the major assistants, which take a question, spend several minutes searching and reading, and return a cited report, are agentic RAG in consumer form. When you use one, you are running a planner-retriever-synthesiser loop, with all the strengths and the 10.2 reliability caveats that implies. We compare the specific Deep Research tools, free tiers included, in 10.5.</p>
  ...
  <ul class="styled-list">
    <li><strong>Context precision &amp; context recall</strong>: did the <em>retrieval</em> step fetch the relevant passages, and only those?</li>
    <li><strong>Faithfulness</strong>: is the <em>generated answer</em> actually supported by the retrieved passages, or did the model add unsupported claims?</li>
    ...
    <table class="comparison-table">
      ...
      <tbody>
        <tr>
          ...
          <td>Long context (paste it in)</td>
        ...
        <tr>
          ...
          <td>Either, then verify by hand</td>
  ...
  <div class="info-box">
    ...
    <p>Messeri &amp; Crockett warned (Week 9.4) that AI can create &ldquo;monocultures of knowing&rdquo;: everyone funnelled toward the same sources and framings. Agentic RAG intensifies the risk, because now the <em>agent</em> decides what to retrieve, what counts as relevant, and what to leave out, and it does so invisibly, in a loop you do not see. A Deep Research report that reads as comprehensive may rest on a narrow, agent-chosen slice of the literature. The friction that retrieval used to add, you deciding what to read, was doing useful epistemic work. Automating it away is convenient and quietly costly.</p>
  </div>
  ...
  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The catch is in the retrieval layer. RAG works by converting text into embeddings (numerical representations) and matching on them, and the embedding models are themselves trained mostly on high-resource languages. If the embedding layer cannot represent isiXhosa or Setswana well, retrieval over an isiXhosa corpus will be poor no matter how good the generating model is. So the equity promise is real but conditional: <strong>RAG can ground frontier models in local knowledge only as well as the retrieval layer handles the local language</strong>, and as of 2026 most embedding models handle African languages poorly. This connects directly to the sovereign-capacity questions of Week 11: the Esethu Framework (Rajab et al., 2025) argues for locally-grounded infrastructure as a precondition for equitable AI rather than an afterthought.</p>
...
SL5_BODY = """
  <div class="intro-text">
    ...
    <p>The centrepiece is a comparison of <strong>Deep Research modes</strong> (the agentic-RAG tools from 10.4) because the Week 10 activity (10.6) hinges on them. We then cover the Chinese free-tier options (often <em>more</em> generous than the Western ones, with one serious catch), the MCP connectors worth wiring into a research workflow, what is expensive and skippable, and a free-tier research pipeline you can actually run.</p>
    <p>A hard warning before any number appears: <strong>every quota in this sub-lesson is a May 2026 snapshot and several will be wrong by the time you read this.</strong> Free tiers are the most volatile thing in the entire field; they change monthly, sometimes weekly. Treat the specific figures as &ldquo;go and check&rdquo;, not gospel. The framework for choosing is the durable part.</p>
  </div>

  <h2 class="section-title">&#127757; Why &ldquo;Free&rdquo; Is the Whole Game Here</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">A course written in California would treat the free-versus-paid question as a budgeting detail. From Cape Town it is structural. The UNDP&#39;s 2025 analysis of African AI talent (drawing on Zindi&#39;s network of some 11,000 data scientists) found that <strong>only about 5% have access to the computing power their work needs</strong>: roughly 1% with on-premise GPUs and 4% paying for cloud access. The continent holds an estimated <strong>0.1% of the world&#39;s computing capacity</strong>, while 60% of the top supercomputers sit in just three countries. The lived effect is stark: a well-resourced researcher in a G7 country can retrain a model every thirty minutes, while an African peer may wait up to six days between iterations.</p>

  <div class="highlight-box">
    <h3>The compute gap is why this week is free-tier-first</h3>
    <p>When 95% of a continent&#39;s AI researchers depend on free or shared tools, &ldquo;just pay for the Pro plan&rdquo; assumes away most of the people this course is for. So everything below is filtered through what is genuinely reachable on a free tier from a South African connection. This is the same equity argument the Esethu Framework (Week 4, expanded in Week 11) makes about infrastructure: access is the precondition, not a detail.</p>
  </div>

  <p style="color: #555; line-height: 1.75; margin-bottom: 15px;">The practical access picture as of May 2026: Claude.ai, ChatGPT, Gemini, Perplexity, and Microsoft Copilot all work from South African networks without a VPN, and UCT&#39;s Microsoft 365 subscription means Copilot Chat is very likely already available to you through your university account. The Chinese services below also work from a Cape Town connection without a VPN. One thing unites all of them, Western and Chinese alike: they process your data on servers outside South Africa, a data-protection point we treat seriously below.</p>

  <h2 class="section-title">&#128270; Deep Research Modes: The Comparison That Matters</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Deep Research modes take a question, spend several minutes planning, searching, and reading, and return a cited report: the consumer face of the agentic RAG from 10.4. They are also what the 10.6 activity asks you to test. Here is the free-tier landscape, with the staleness warning in force.</p>

  <div class="warning-box">
    <h4>&#9888;&#65039; Quotas below are a May 2026 snapshot &mdash; verify before relying</h4>
    <p>These numbers move constantly. While this was being written, sources already disagreed about Perplexity&#39;s free Deep Research allowance (some said five a day, one said none), which tells you how much weight to put on any single figure. Check the current limit yourself before building the activity around a tool.</p>
  </div>
    ...
    <table class="comparison-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>Moonshot AI. The strongest free <em>alternative</em> to Perplexity: cited, visualised reports.</td>
        </tr>
        <tr>
  ...
  <p style="color: #555; line-height: 1.75; margin-top: 18px; margin-bottom: 15px;">For the 10.6 activity, the recommended free Deep Research tool is <strong>Perplexity</strong>; if its quota is exhausted or sign-up is blocked, <strong>Kimi</strong> is the strongest substitute, provided you read the data-protection note next and disclose your choice as the activity requires.</p>

  <h2 class="section-title">&#127760; Chinese Free Tiers: Often More Generous</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">The Western free-tier list is West-Coast-heavy, and for a student who cannot pay, that matters: the major Chinese services are often <em>more</em> generous, accept email or Google sign-up, and (as of May 2026) work from a Cape Town connection without a VPN. They belong in any honest free-tier map. The honest ranking for the UCT student who cannot pay:</p>
  ...
  <div class="card-grid">
    ...
    <div class="card">
      ...
      <p>Moonshot AI. The closest free analogue to Perplexity Deep Research, plus document and slide generation and a very long (256K+) context window. Google sign-in, English UI. Free Deep Research quota is not published; expect it to tighten.</p>
    </div>
    <div class="card">
      <h3>3. Qwen &mdash; chat.qwen.ai</h3>
      <p>Alibaba. The widest <em>modality</em> coverage on a free tier: hybrid thinking, web search, image/video understanding, image generation. Note: Qwen&#39;s free coding-agent CLI was discontinued on 15 April 2026; the chat surface stays free, the coding CLI now needs payment.</p>
    </div>
    <div class="card">
  ...
  <div class="warning-box">
    ...
    <p>It is tempting to attach the data-protection worry only to the Chinese services, but that would be both unfair and wrong. <strong>Almost every free AI tool in this sub-lesson processes your data outside South Africa</strong>: the US-based services (ChatGPT, Claude, Gemini, Perplexity) on infrastructure outside South Africa, typically in the United States; the Chinese services (DeepSeek, Kimi, Qwen, Z.ai) on servers in mainland China. Under South Africa&#39;s Protection of Personal Information Act (POPIA, section 72), the question is whether sending personal information abroad is permitted at all, not which country it goes to: a transfer abroad is restricted unless the recipient is bound by data-protection rules substantially similar to POPIA&#39;s, or the data subject has consented (among a few narrow conditions). POPIA has no &ldquo;approved-country&rdquo; whitelist; the test is the recipient&#39;s actual protection, which for any consumer AI service hosting your prompts abroad you have no basis to assume is met, wherever its servers sit.</p>
    <p>Applied equally to the US-hosted and the China-hosted tools: <strong>do not put identifiable personal data, participant information, unpublished results, or third-party confidential material into any of them.</strong> For public, non-personal research questions they are all excellent and free. For anything involving real people&#39;s data, the obligation is the same regardless of vendor, and you must record which tool you used and where its data went. The 10.6 activity makes this disclosure a required, graded section, for every tool, not an afterthought.</p>
  </div>
  ...
  <ul class="styled-list">
    <li><strong>Enterprise agents</strong> (Sierra, Decagon, Cognition&#39;s Devin): priced for companies; not aimed at, or affordable for, an individual researcher.</li>
    <li><strong>Manus</strong>: the &ldquo;general agent&rdquo; from 10.3. Access is mixed and its capability claims are company-reported; not something to build a workflow on now.</li>
    <li><strong>Cursor Pro</strong>: genuinely good if you code every day; if you do not, the free coding tools cover it.</li>
    <li><strong>Paid Deep Research tiers</strong>: faster and higher-quota, but the free tiers are enough to do real work and to complete this course. The activity is built to be done entirely free.</li>
  </ul>

  <div class="info-box">
    <h4>&#127757; A Week 3 reminder: agents are expensive in tokens, not just money</h4>
    <p>An agentic run (planning, many tool calls, reading long results, retrying) burns far more tokens than a single chat message, sometimes by orders of magnitude. That is why free Deep Research is rationed, and it is also a Week 3 environmental point: a Deep Research report has a much larger compute (and therefore energy and water) footprint than a one-shot answer. Use the heavy tools when the task warrants them, not reflexively.</p>
  </div>

  <h2 class="section-title">&#129534; A Free-Tier Research Pipeline</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Putting the week together, here is a five-step pipeline a UCT student can run entirely on free tools, and which keeps you in the loop at every verification point:</p>

  <ul class="step-list">
    <li><strong>Explore</strong>: run your question through a free Deep Research mode (Perplexity, or Kimi for non-personal topics) to map the landscape and surface sources. (Week 10.4)</li>
    <li><strong>Verify</strong>: do not trust the report. Run every citation through the Week 5 Five-Point Citation Check and the Week 9 &ldquo;which model, which date, retested?&rdquo; check. Expect some sources to be wrong or invented.</li>
    <li><strong>Synthesise</strong>: organise what survived verification into your own structure, in your own words. The work of deciding what matters and how it fits together is yours; a chat tool can react to your structure, not replace it. (Week 6)</li>
    <li><strong>Write</strong>: write your own first draft. Then use AI to refine it, tightening, questioning, suggesting, checking each change as you go. This is the Week 6 order, and it matters: the thinking and the first words should be yours, with AI sharpening what you wrote, not generating it for you. Drafting by prompt is how you end up with fluent text you do not actually understand or stand behind.</li>
    <li><strong>Audit</strong>: before anything leaves your hands, check it as you would AI-generated code: claims against sources, numbers against primaries, and a record of which tool did what. (Weeks 7 and 9)</li>
  </ul>
  ...
  <div class="intro-text" style="margin-top: 50px;">
    <h2>&#128073; What Comes Next</h2>
    <p><strong>Sub-Lesson 10.6 &mdash; Hands-On Activities and Assessment.</strong> Now you put it to work. The headline activity, &ldquo;Same Task, Three Ways&rdquo;, has you run a research question from your own field through plain chat, chat-with-tools, and a Deep Research mode, then judge the results with the Week 9 failure taxonomy and verify them with the Week 5 citation checks, using free tools only and disclosing where your data went. It is the whole week, made concrete on a question you actually care about.</p>
  </div>
"""
...
SL6_BODY = """
  <div class="intro-text">
    ...
    <p>The headline activity is <strong>&ldquo;Same Task, Three Ways&rdquo;</strong>: run one question through plain chat, chat-with-tools, and a Deep Research mode, then judge the results with the Week 9 failure taxonomy and verify them with the Week 5 citation checks. Two shorter activities follow, and the week&#39;s assessment pulls them together. Everything here is designed to be done <strong>entirely on free tools</strong>, on a phone or a borrowed laptop if that is what you have, because, as 10.5 argued, a course for this continent cannot assume a subscription.</p>
    <p>One last echo of Week 9: every output you produce this week is a snapshot dated May 2026. Part of the assessment is acknowledging that: recording what was true when you did the work, and how soon you would expect it to change.</p>
  </div>

  <h2 class="section-title">&#129514; Activity 1: Same Task, Three Ways</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">The core activity. Choose a research question <strong>in your own field</strong>: specific, genuinely answerable, and not trivially Googleable. Something a knowledgeable colleague would have to think about. You will be the expert judge of the answers, which is the point: you can tell when an AI is wrong in your own area in a way you cannot in someone else&#39;s.</p>
  ...
  <ul class="step-list">
    <li><strong>Plain chat</strong>: a free assistant with no tools (Claude.ai, ChatGPT, or Gemini free; pick one and stay with it). The model answers from training alone.</li>
    <li><strong>Chat with tools</strong>: the same kind of assistant, but with web search / browsing switched on, so it can retrieve before answering.</li>
    <li><strong>Deep Research mode</strong>: a full agentic-RAG run. Perplexity&#39;s free Deep Research is the recommended default. <em>If its quota is exhausted or sign-up is blocked, Kimi (kimi.com) is an acceptable substitute</em>, but see the data-disclosure rule below, which applies whichever tool you choose.</li>
  </ul>

  <p style="color: #555; line-height: 1.75; margin-top: 15px; margin-bottom: 15px;">Then write a <strong>one-to-two-page comparison</strong>. Do not just say which was &ldquo;best&rdquo;. For each mode, record: how deep and specific the answer was; how good the citations were (and whether they exist, which you will check in Activity 2); what it got wrong in your expert judgement; and where each mode failed. Then do the analytical core of the exercise:</p>

  <div class="info-box">
    <h4>&#128202; Apply the Week 9.2 taxonomy explicitly</h4>
    <p>For every failure you observed, classify it: was it <strong>patched</strong> (you were using a weak tool; a current one would not fail this way), <strong>reduced-but-persistent</strong> (a known weakness you can manage with better prompting or tool choice), or <strong>structural</strong> (something the next model release will not fix: long-tail gaps, compositional error, the reliability-not-accuracy problem from 10.2)? Then, for the structural ones, state what the Week 9.5 verification protocol would have you do about it. This is the muscle the whole activity exists to build.</p>
  </div>
  ...
  <p style="color: #555; line-height: 1.75; margin-top: 15px; margin-bottom: 15px;">Then report the numbers: of the citations the Deep Research tool gave you, how many checked out completely? How many pointed to real sources that said something <em>different</em> from the report&#39;s claim? How many were to &ldquo;papers&rdquo; that do not appear to exist at all? This is the Week 5 hallucinated-citation exercise carried into the agentic-RAG era, and the results are usually sobering. A fluent, well-formatted, confident research report is not a verified one.</p>

  <h2 class="section-title">&#128268; Activity 3 (Optional): A Small MCP Workflow</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">For students who want to go further. Using one of Claude.ai&#39;s free connectors (the creative connectors, or, since April 2026, the read-only Microsoft 365 connector) wire up <em>one</em> small step of your research workflow, involving nothing personal or confidential. Write 250 words on what worked, what failed, and, above all, <strong>what you would never let it do unsupervised</strong>, and why. The point is not the connector; it is articulating your own permissions dial (10.1) for a real task.</p>

  <div class="info-box">
    <h4>&#128274; A note on the Microsoft 365 connector</h4>
    <p>It needs a <em>business or education</em> Microsoft account (not a personal @outlook.com one), and your institution&#39;s IT must allow the connection. There is a simple way to find out whether yours does: try adding it. If Microsoft shows an &ldquo;administrator approval required&rdquo; screen, self-service connection is not enabled for your institution and you would need IT to approve it. Reading that consent screen, and deciding whether you would even want to grant the access it asks for, is itself a useful exercise in the permissions thinking this week is about.</p>
  </div>

  <h2 class="section-title">&#127760; The Data-Disclosure Rule (Required, Graded)</h2>

  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Every tool in this activity processes your data outside South Africa, the US-based assistants and the Chinese ones alike (10.5). So a disclosure statement is part of the deliverable, not an optional extra. Two firm rules and a template:</p>
  ...
  <ul class="styled-list">
    ...
    <li><strong>Disclose what you used and where it went.</strong> Naming your tools and their data destinations is a habit worth building now; it is what a research-ethics committee or a journal will increasingly expect.</li>
  </ul>
  ...
  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">The assessment is a single piece of roughly <strong>1,500 words</strong>, in the same spirit as the Week 9 assessment: an explicitly dated snapshot that acknowledges its own coming obsolescence. <strong>Free tools only</strong>, a hard rule, so that everyone is judged on the same playing field regardless of what they can afford. Required sections:</p>
  ...
  <ul class="styled-list">
    <li><strong>An agent is a model plus a harness</strong> (tools, a loop, memory, and permissions), and since 2024 the harness, not the model, increasingly decides how well the whole thing works (10.1).</li>
    ...
    <li><strong>The tool landscape is real but volatile</strong> (coding, computer-use, browser, and research agents, connected increasingly through MCP) and best read with three questions: which harness, how reliable, free from here? (10.3)</li>
    ...
    <li><strong>A great deal is genuinely free</strong> from South Africa, Western and Chinese alike, if you know where to look and you treat cross-border data flow seriously for every tool (10.5).</li>
  </ul>
...
SUBLESSONS = [
    {
        ...
        "header_subtitle": "A working definition of agents for researchers, and the shift since 2024: the harness, not the model, has become the product",
        "body": SL1_BODY,
    },
    {
        ...
        "header_subtitle": "Applying the Week 9 patched / reduced / structural taxonomy to agents, and the finding that reliability is not accuracy",
        "body": SL2_BODY,
    },
    {
        ...
        "header_subtitle": "A time-stamped, free-tier-honest tour of coding agents, computer-use and browser agents, and the MCP connector layer, as of May 2026",
        "body": SL3_BODY,
    },
    {
        ...
        "header_subtitle": "What changed since Week 5: long context vs retrieval, agentic RAG and Deep Research, evaluating it all, and why the researcher stays in the loop",
        "body": SL4_BODY,
    },
    {
        ...
        "header_subtitle": "An honest free-versus-paid map of Deep Research modes, Chinese free tiers, MCP connectors and a free research pipeline, built for a student without a foreign credit card",
        "body": SL5_BODY,
    },
    {
        ...
        "header_subtitle": "Same Task, Three Ways: test plain chat, chat-with-tools, and Deep Research on your own research question, then judge and verify the results, on free tools only",
        "body": SL6_BODY,
    },
]
