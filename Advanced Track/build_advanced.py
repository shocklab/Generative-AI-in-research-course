...
Fixed in advance: outlier rule (exclude only samples the field notes mark
  ...
  <div class="instructor-box">
    <h4>&#128100; From the instructor&#39;s own practice</h4>
    <p>&ldquo;Before I spend real compute I pre-register: I write down what &lsquo;interesting&rsquo;, &lsquo;boring-but-worth-knowing&rsquo;, and &lsquo;dead&rsquo; each look like, and a decision rule &mdash; then I hold myself to it even when the result disappoints. The pre-registration is binding; outcomes don&#39;t get reframed after the fact. It is uncomfortable exactly when it is working: the discomfort of writing down a clean STOP when a pre-registered gate fails is the whole value of having written it.&rdquo;</p>
...
SL_B3_BODY = """
  ...
  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">When the agent runs for an hour and writes a script you did not watch line by line, verification is harder and matters more &mdash; Week&nbsp;9&#39;s protocols and Week&nbsp;10&#39;s Princeton reliability finding apply with full force. The good news is that the reproducible structure gives you the surfaces you need to check the work; the honest news is that some of the burden simply stays human.</p>

  <div class="card-grid">
    <div class="card">
      <h3>Plan before consequence</h3>
      <p>Keep plan mode in front of any step you cannot easily reverse. Approving a plan is far cheaper than unpicking a bad action, and it is the checkpoint where most errors are caught before they happen.</p>
    </div>
    <div class="card">
      <h3>Read the diff</h3>
      <p>Before accepting a change, look at what actually changed. The <code>CLAUDE.md</code> rule &ldquo;show me the diff before done&rdquo; exists so this is routine, not optional.</p>
    </div>
    <div class="card">
      <h3>Use the decision log as a check</h3>
      <p>The log is not just a record &mdash; it is a verification surface. Read the decisions back: does each one make sense, is each sourced, would you have made it? A decision that cannot be justified in the log is a decision to revisit.</p>
    </div>
    <div class="card">
      <h3>Re-run and spot-check</h3>
      <p>Run the script yourself and confirm you get the same numbers. Trace a few output values back to the raw data by hand. An independent verification subagent (B.2) can do a first pass, but your own spot-check is the one that counts.</p>
    </div>
  </div>
