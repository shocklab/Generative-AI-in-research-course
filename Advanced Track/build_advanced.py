...
SL_A3_BODY = """
  ...
  <div class="info-box">
    <h4>&#128230; The Berg River sample files</h4>
    <p>The downloadable Berg River archive ships with <strong>Lesson&nbsp;B</strong>, where the same files drive a full reproducible analysis &mdash; it is being prepared as one coherent bundle together with the <code>CLAUDE.md</code> template and the example Skills, so that everything you download fits together. Until then you have two good options: read the steps below as a walk-through, or &mdash; better &mdash; point Claude Code at <em>a real folder of your own</em> that could use tidying. A messy archive from one of your own projects works just as well for this first exercise, provided you have a backup and you keep the agent in inspect-only mode throughout.</p>
  </div>
