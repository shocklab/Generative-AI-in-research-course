...
- British spelling.</pre>
  ...
  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">This is the habit that most separates careful computational research from the other kind, and it goes beyond anything in the source workshop. The idea is simple and old &mdash; it comes from clinical trials, and the case for making it a routine research habit rather than a clinical-trials formality is laid out in Nosek et&nbsp;al.&#39;s <em><a href="https://doi.org/10.1073/pnas.1708274114" target="_blank" rel="noopener">The Preregistration Revolution</a></em> (2018, PNAS) &mdash; and it transfers directly to any experiment where you can run a cheap pilot: <strong>write down what you expect to find, and the rule for deciding, before you run the thing.</strong></p>
...
SL_B3_BODY = """
  ...
  <p style="color: #555; font-size: 1.05em; margin-bottom: 20px; line-height: 1.8;">Here is the Berg River study, start to finish, with the discipline of the previous pages applied. The dataset is the same deliberately-messy archive from Lesson&nbsp;A.3 &mdash; downloadable below &mdash; and it has real problems planted in it: inconsistent column names, a systematic counting fault at one site, a contaminated outlier, missing values, and field notes that explain anomalies the spreadsheets don&rsquo;t. Watch how the structure turns that mess into something inspectable &mdash; including one correction that, it turns out, decides the answer.</p>
  ...
  <ol class="step-list">
    ...
    <li><strong>Inspect and inventory.</strong> In plan mode, the agent reads every raw file and writes <code>docs/data-inventory.md</code>: the columns (and the fact that the three site files name them differently), the units, the row counts, the missing values, and the anomalies &mdash; including upstream counts that sit suspiciously close to the town&rsquo;s (surprising for a site <em>above</em> the town), and one downstream value far higher than its neighbours. Crucially, <code>field-notes.md</code> explains both: the upstream counter double-counted all season, and the high downstream sample (D-03) was contaminated and should be discarded.</li>
    ...
    <li><strong>Record assumptions and uncertainty.</strong> The agent notes what it assumed and what remains uncertain &mdash; the small sample after exclusions, the reliance on a single field note for the upstream correction that drives the result &mdash; rather than presenting a clean result as if nothing were in doubt.</li>
